...
$query = "SELECT * FROM sales WHERE date BETWEEN '$d1' AND '$d2' ORDER BY transaction_id DESC";
...
if ($result = $conn->query($query)) {
    ...
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            ...
            echo '<td>' . (isset($row['transaction_id']) ? 'STI-00' . $row['transaction_id'] : '') . '</td>';
            echo '<td>' . (isset($row['date']) ? $row['date'] : '') . '</td>';
            echo '<td>' . (isset($row['name']) ? $row['name'] : '') . '</td>';
            echo '<td>' . (isset($row['invoice_number']) ? $row['invoice_number'] : '') . '</td>';
            echo '<td>' . (isset($row['amount']) ? formatMoney($row['amount'], true) : '') . '</td>';
            echo '<td>' . (isset($row['profit']) ? formatMoney($row['profit'], true) : '') . '</td>';
            echo '</tr>';
        }
    } else {
...
$results = $conn->prepare("SELECT sum(amount) FROM sales WHERE date BETWEEN :a AND :b");
$resultia = $conn->prepare("SELECT sum(profit) FROM sales WHERE date BETWEEN :c AND :d");

if ($results && $resultia) { // Check if preparation was successful
    ...
    if ($results === false) {
        die("Query for sum(amount) failed: " . $conn->error);
    }
    ...
    if ($resultia === false) {
        die("Query for sum(profit) failed: " . $conn->error);
    }
