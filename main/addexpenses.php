...
<form action="saveexpenses.php" method="post">
    ...
    <div id="ac">
        ...
        <select name="expense_type" style="width:265px; height:30px; margin-left:-5px;">
            <option></option>
            <?php
            include('../connect.php');

            // Prepare and execute the query
            $result = $conn->prepare("SELECT expense_type FROM expensescategory");
            $result->execute();

            // Bind the result to a variable
            $result->bind_result($expense_type);

            // Create the dropdown options
            while ($result->fetch()) {
                echo '<option>' . $expense_type . '</option>';
            }
            ...
            <span>Amount Spent: </span><input type="text" id="txt1" style="width:265px; height:30px;" name="amount_spent" Required><br>
